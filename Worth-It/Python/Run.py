...
Final_Lib = array_to_table(final_array).sample(with_replacement=False)
...
while True:
    ...
    exact = float(np.array(Final_Lib.row(i)).item(3))
    error_array = np.append(error_array, "%" + str(((approximation - exact) / exact)*100))
    i += 1
...
output = Final_Lib.join("Menu Item", error_table).sort("Error", descending=True)
print(output)
